...
void jpeg_write(std::vector<libcamera::Span<uint8_t>> const &mem, StreamInfo const &info, ControlList const &metadata,
                std::basic_ostringstream<char> &obuff, std::string const &cam_model, StillOptions const *options)
{
    ...
    try {
        ...
        obuff.write(reinterpret_cast<char const *>(exif_header), sizeof(exif_header) * 1);
        obuff.put(static_cast<char>((exif_len + thumb_len + 2) >> 8));
        obuff.put(static_cast<char>((exif_len + thumb_len + 2) & 0xff));
        obuff.write(reinterpret_cast<char const *>(exif_buffer), exif_len * 1);
        if (thumb_len) {
            obuff.write(reinterpret_cast<char const *>(thumb_buffer), thumb_len * 1);
        }
        obuff.write(
			reinterpret_cast<char const *>(jpeg_buffer + exif_image_offset),
			(jpeg_len - exif_image_offset) * 1
		);
