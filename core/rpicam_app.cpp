...
void RPiCamApp::ConfigureStill(unsigned int flags)
{
    ...
    if(have_lores_stream){
        ...
        configuration_->at(2).pixelFormat = lores_format_;
        configuration_->at(2).size = lores_size;
        configuration_->at(2).bufferCount = configuration_->at(0).bufferCount;
        configuration_->at(2).colorSpace = configuration_->at(0).colorSpace;
    }
    ...
    if(have_lores_stream){
        streams_["lores"] = configuration_->at(2).stream();
    }
